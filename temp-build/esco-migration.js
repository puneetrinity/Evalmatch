...
export class ESCOMigration {
    ...
    `);
        for (const skill of ESCO_SKILLS_DATA) {
            ...
            const safeCategory = toCategory(processedSkill.category);
            await insertStmt.run(processedSkill.escoId, processedSkill.skillTitle, processedSkill.alternativeLabel, processedSkill.description, safeCategory, processedSkill.subcategory, processedSkill.domain, processedSkill.reuseLevel, processedSkill.skillType, processedSkill.conceptUri, processedSkill.status);
        }
     ...
     */
    async applyContaminationGuards() {
    ...
    `);
        for (const [guardName, guardData] of Object.entries(CONTAMINATION_GUARDS)) {
            for (const pattern of guardData.patterns) {
                await insertGuard.run(guardName, pattern.source, guardData.allowedContexts.join(','), guardData.blockInDomains.join(','), 0.95);
            }
        }
 ...
 */
export async function runESCOMigration() {
