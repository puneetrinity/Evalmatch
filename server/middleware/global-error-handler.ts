 ...
 */
function categorizeError(error: AppError): {
  statusCode: number;
  errorType: string;
} {
...
): AppError => createError(message, 401, "AUTHENTICATION_ERROR");

export const createForbiddenError = (
  message: string = "Access forbidden",
): AppError => createError(message, 403, "AUTHORIZATION_ERROR");

export const createNotFoundError = (
  message: string = "Resource not found",
): AppError => createError(message, 404, "NOT_FOUND_ERROR");

export const createConflictError = (
  message: string,
  details?: Record<string, unknown>,
): AppError => createError(message, 409, "CONFLICT_ERROR", details);

export const createRateLimitError = (
  message: string = "Rate limit exceeded",
): AppError => createError(message, 429, "RATE_LIMIT_ERROR");

export const createDatabaseError = (
  message: string,
  details?: Record<string, unknown>,
): AppError => createError(message, 503, "DATABASE_ERROR", details);
