...
export async function initializeDatabase(): Promise<void> {
  if (!config.database.enabled) {
    logger.info('Database disabled in configuration - will use memory storage fallback');
    return;
  }
  ...
  if (!config.database.url) {
    ...
    if (config.env === 'production') {
      logger.error('CRITICAL: Production environment requires valid database configuration');
      throw new Error(`${errorMsg} - check environment variables`);
    } else {
      logger.warn('Development environment missing database URL - this will cause connection failures');
  ...
  pool.on('connect', (client) => {
    ...
    client.on('error', (err) => {
      logger.error('PostgreSQL client error:', {
        ...
        connectionInfo: {
          processID: 'processID' in client ? (client as any).processID : undefined,
...
export function getDatabase() {
  if (!db) {
    ...
    if (!config.database.enabled) {
      throw new Error('Database is disabled in configuration - enable database or use memory storage methods');
    }
    throw new Error('Database not initialized - database connection failed during startup');
  }
  return db;
}
...
function handleConnectionError(error: Error): void {
  ...
  if (circuitBreaker.failureCount >= CIRCUIT_BREAKER.FAILURE_THRESHOLD) {
    ...
    circuitBreaker.nextAttemptTime = Date.now() + CIRCUIT_BREAKER.RETRY_INTERVAL_MS;
    logger.warn(`Circuit breaker opened due to ${circuitBreaker.failureCount} failures`);
  }
...
function checkCircuitBreaker(): void {
  if (circuitBreaker.state === 'OPEN') {
    if (Date.now() >= circuitBreaker.nextAttemptTime) {
      ...
      throw new Error('Circuit breaker is open - database connections temporarily unavailable');
    }
  }
}
...
export async function executeQuery<T = unknown>(query: string, params?: unknown[]): Promise<T[]> {
  if (!pool) {
    if (!config.database.enabled) {
      throw new Error('Database queries not available - database is disabled in configuration');
    }
    throw new Error('Database connection not available - initialization may have failed');
  }
  ...
  try {
    ...
    const queryTimeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => reject(new Error('Query execution timeout')), 
...
function trackQueryPerformance(queryTime: number): void {
  ...
  // Update statistics
  stats.averageQueryTime = queryTimes.reduce((sum, time) => sum + time, 0) / queryTimes.length;
...
function startPeriodicHealthChecks(): void {
  ...
  logger.info(`Started periodic health checks every ${HEALTH_CHECK_INTERVAL/1000}s`);
}
...
export async function validateConnection(retries = 3): Promise<boolean> {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      logger.warn(`Connection validation failed (attempt ${attempt}/${retries}):`, error);
...
export function getPoolHealth(): {
  healthy: boolean;
  metrics: {
    connections: { total: number; idle: number; active: number; waiting: number };
    performance: { avgQueryTime: number; maxQueryTime: number; slowQueries: number };
    errors: { connectionFailures: number; queryFailures: number; timeouts: number };
...
process.on('unhandledRejection', async (reason, promise) => {
  logger.error('Unhandled rejection - database may be affected:', { reason, promise });
...
function setCachedQuery<T = unknown>(cacheKey: string, result: T[], ttlMs = 60000): void {
...
export async function executePreparedStatement<T = unknown>(
  ...
  try {
    ...
    const result = await client.query(`EXECUTE ${name}${params ? `(${params.map((_, i) => `$${i + 1}`).join(', ')})` : ''}`, params);
    return result.rows;
      ...
      warmUpPromises.push(
        (async () => {
          ...
          try {
            ...
            await client.query('SELECT 1 as warmup_test, NOW() as connected_at');
            logger.debug(`Connection ${i + 1}/${minConnections} warmed up successfully`);
          } finally {
            client.release();
          }
        })()
...
async function validatePoolConfiguration(pool: Pool): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      if (acquireTime > 1000) {
        logger.warn(`⚠️  Slow connection acquisition: ${acquireTime}ms (expected < 1000ms)`);
      }
      
      if (queryTime > 100) {
        logger.warn(`⚠️  Slow query execution: ${queryTime}ms (expected < 100ms for simple queries)`);
      }
