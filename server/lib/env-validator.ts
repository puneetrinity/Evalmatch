...
interface EnvVarSpec {
  name: string;
  required: boolean;
  category: 'core' | 'database' | 'firebase' | 'ai' | 'security' | 'performance';
...
const ENV_SPECS: EnvVarSpec[] = [
  ...
  },
  {
    ...
    required: true,
    category: 'firebase',
    validator: (val) => val.includes('.firebaseapp.com') || val.includes('.web.app'),
  ...
  // Deployment Configuration
  {
    ...
    required: false,
    category: 'security',
    validator: (val) => val.split(',').every(origin => origin.trim().startsWith('http')),
  ...
  },
  {
    ...
    required: false,
    category: 'core',
    validator: (val) => val.includes('.railway.app') || val.includes('.up.railway.app'),
  ...
  // Performance & Configuration
  {
    ...
    validator: (val) => !isNaN(parseInt(val)) && parseInt(val) > 0 && parseInt(val) <= 10,
...
export function validateEnvironment(): EnvValidationResult {
  ...
  for (const spec of ENV_SPECS) {
    ...
    const isRequired = spec.required || (isProduction && spec.productionRequired);
  ...
  if (!hasAnyAIProvider && !authBypassMode) {
    errors.push({
      ...
      category: 'critical',
      suggestion: 'Set one of: GROQ_API_KEY, OPENAI_API_KEY, or ANTHROPIC_API_KEY'
    });
  ...
  if (firebaseProjectId && viteFirebaseProjectId && firebaseProjectId !== viteFirebaseProjectId) {
    errors.push({
...
export function displayValidationResults(result: EnvValidationResult): void {
  const { isValid, errors, warnings, missingCritical, missingOptional } = result;

  if (isValid && warnings.length === 0) {
    logger.info('✅ Environment validation passed - all required variables configured correctly');
    return;
  }
  ...
  if (errors.length > 0) {
    ...
    // Group by category
    const categories = {
      core: ENV_SPECS.filter(s => s.category === 'core' && missingCritical.includes(s.name)),
      firebase: ENV_SPECS.filter(s => s.category === 'firebase' && missingCritical.includes(s.name)),
      ai: ENV_SPECS.filter(s => s.category === 'ai'),
      security: ENV_SPECS.filter(s => s.category === 'security' && missingCritical.includes(s.name)),
    };
    ...
    Object.entries(categories).forEach(([categoryName, specs]) => {
      if (specs.length > 0 || categoryName === 'ai') {
        ...
        if (categoryName === 'ai') {
          ...
          logger.error('   GROQ_API_KEY=gsk_your_groq_key_here          # Fastest, recommended');
          logger.error('   OPENAI_API_KEY=sk-your_openai_key_here       # Reliable fallback');
          logger.error('   ANTHROPIC_API_KEY=sk-ant-your_key_here       # High quality');
        } else {
          specs.forEach(spec => {
            logger.error(`   ${spec.name}=${spec.example || '[required_value]'}    # ${spec.description}`);
          });
        }
        logger.error('');
      }
    ...
    logger.error('SESSION_SECRET=your-very-secure-random-string-32-chars-minimum');
    logger.error('');
  }
}
...
export function validateEnvironmentOrExit(): EnvValidationResult {
  ...
  if (!result.isValid && !authBypassMode) {
    logger.error('💥 Application cannot start due to environment configuration errors.');
    logger.error('Please fix the above issues and restart the application.');
    logger.error('');
    logger.error('ℹ️  For testing purposes, you can set AUTH_BYPASS_MODE=true to bypass some validations.');
    process.exit(1);
  } else if (!result.isValid && authBypassMode) {
    logger.warn('⚠️  Starting with configuration errors due to AUTH_BYPASS_MODE=true');
