...
export class DatabaseRateLimiter {
  ...
  constructor(config?: Partial<RateLimiterConfig>) {
    ...
    console.log(`- Max requests: ${this.config.maxRequestsPerInterval} per ${this.config.intervalMs}ms`);
   ...
   */
  async execute<T>(operation: () => Promise<T>, context = 'db operation'): Promise<T> {
    ...
    const maxAllowedRequests = this.config.maxRequestsPerInterval * this.config.burstFactor;
    ...
    if (this.consecutiveThrottles > 3) {
      console.warn(`Rate limiting database operations: ${currentRate}/${this.config.maxRequestsPerInterval} requests, delaying ${delay}ms`);
    }
   ...
   */
  private queueOperation<T>(operation: () => Promise<T>, context: string): Promise<T> {
    ...
    if (this.pendingOperations.length >= this.config.maxQueueSize) {
      ...
      return Promise.reject(new Error(`Database operation queue full (${this.config.maxQueueSize} max). Too many concurrent operations.`));
    }
    ...
    if (currentRate < this.config.maxRequestsPerInterval) {
      ...
      if (next) {
        ...
        if (this.pendingOperations.length > 10) {
          console.log(`Processing database operation queue: ${this.pendingOperations.length} remaining operations`);
        }
      ...
      if (this.pendingOperations.length > 0) {
        console.log(`Rate limited: waiting ${waitTime}ms before processing ${this.pendingOperations.length} queued database operations`);
      }
